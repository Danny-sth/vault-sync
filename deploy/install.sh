...
[ -f "$DEPLOY/.env" ] || { echo "нет $DEPLOY/.env (шаблон: .env.example, копия — в вольте Creds)" >&2; exit 1; }
...
log "проверки"
sleep 3
code() { curl -s -o /dev/null -w '%{http_code}' --resolve "$DOMAIN:443:127.0.0.1" "$@"; }
...
DEC=$(cd "$REPO/server/scripts" && node vault-cli.mjs list "" </dev/null 2>/dev/null | wc -l)
